...
<body>
		...
		<div class="panel admin-panel">
			<div class="panel-head">
				<strong class="icon-reorder"><span style="font-size:16px;color:blue;"></span>&nbsp;&nbsp;必修课学生列表</strong> 
			</div>
...
<form  action="/elective/DaFen" method="post">
	<c:forEach var="kecheng" items="${kecheng}" varStatus="status">
				 <c:forEach var="stu" items="${kecheng.stu}" varStatus="status">
				 ...
				 <tr>
					...
					<td>${stu.sphone}</td>
					<td>${kecheng.cname}</td>
					<td><input type="text" value="${kecheng.ccredit}" name="ccredit" style="display: none;">${kecheng.ccredit}</td>
					<td>${kecheng.ctype}</td>
					 <td>
					   <input type="text" value="${kecheng.cid}" name="cid" style="display: none;">
